...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  return requireAuth(async (req, res, user) => {
    if (req.method === 'GET') {
      if (!['DISPATCHER', 'SAFETY_OFFICER', 'FLEET_MANAGER', 'ADMIN'].includes(user.role)) {
      ...
      try {
        ...
        if (vehicle.status === 'IN_SHOP')
          return res.status(400).json({ message: 'In Shop vehicles cannot be dispatched' });
        if (vehicle.status === 'ON_TRIP')
          return res.status(400).json({ message: 'Vehicle is already on a trip' });
        if (Number(data.weight) > vehicle.capacity)
          return res.status(400).json({ message: 'Weight exceeds vehicle capacity' });
        ...
        if (driver.status === 'ON_TRIP')
          return res.status(400).json({ message: 'Driver already On Trip cannot be reused' });
        if (new Date(driver.licenseExpiry) < new Date())
          return res.status(400).json({ message: 'Expired license cannot be assigned' });
