...
export default function FuelExpenses() {
  ...
  const totalFuelCost = fuelLogs.reduce(
    (acc: number, log: { cost?: number }) => acc + (log.cost || 0),
    0
  );
  const totalMaintCost =
    maintenanceLogs.reduce((acc: number, log: { cost?: number }) => acc + (log.cost || 0), 0) +
    expenses
      .filter((e: { type: string }) => e.type === 'MAINTENANCE' || e.type === 'REPAIR')
      .reduce((acc: number, e: { amount: number }) => acc + e.amount, 0);

  const totalOpCost = totalFuelCost + totalMaintCost;

  // Group expenses by Trip
  const tripExpensesMap = expenses.reduce(
    (
      acc: Record<
        string,
        {
      ...
      if (!acc[tripId]) {
        acc[tripId] = {
          tripId,
          tripName: exp.trip
            ? `TR${exp.trip.id.substring(exp.trip.id.length - 4).toUpperCase()}`
            : 'Unlinked',
  ...
  return (
    <>
      ...
      <div className="space-y-6">
                ...
                <form onSubmit={handleFuelSubmit} className="space-y-4">
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) => val && setFuelForm({ ...fuelForm, vehicleId: val })}
                      required
                    >
                      <SelectTrigger>
                ...
                <form onSubmit={handleExpenseSubmit} className="space-y-4">
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        val && setExpenseForm({ ...expenseForm, tripId: val === 'none' ? '' : val })
                      }
                    >
                      <SelectTrigger>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        val &&
                        setExpenseForm({ ...expenseForm, vehicleId: val === 'none' ? '' : val })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select vehicle" />
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) => val && setExpenseForm({ ...expenseForm, type: val })}
                      required
                    >
                      <SelectTrigger>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg uppercase text-muted-foreground tracking-wider font-semibold">
              Fuel Logs
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {fuelLoading ? (
                    ...
                    fuelLogs
                      .slice((fuelPage - 1) * ITEMS_PER_PAGE, fuelPage * ITEMS_PER_PAGE)
                      .map(
                        (log: {
                          id: string;
                          vehicle?: { registration: string };
                  ...
                  <PaginationContent>
                    ...
                    <PaginationItem>
                      <span className="text-sm px-2">
                        Page {fuelPage} of {Math.ceil(fuelLogs.length / ITEMS_PER_PAGE)}
                      </span>
                    </PaginationItem>
                    <PaginationItem>
                      <PaginationNext
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          setFuelPage((p) =>
                            Math.min(Math.ceil(fuelLogs.length / ITEMS_PER_PAGE), p + 1)
                          );
                        }}
                        className={
                          fuelPage === Math.ceil(fuelLogs.length / ITEMS_PER_PAGE)
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {expensesLoading ? (
                    ...
                    tripExpenses
                      .slice((expensePage - 1) * ITEMS_PER_PAGE, expensePage * ITEMS_PER_PAGE)
                      // eslint-disable-next-line @typescript-eslint/no-explicit-any
              ...
              <div className="mt-4">
                <Pagination>
                  <PaginationContent>
                    ...
                    <PaginationItem>
                      <span className="text-sm px-2">
                        Page {expensePage} of {Math.ceil(tripExpenses.length / ITEMS_PER_PAGE)}
                      </span>
                    </PaginationItem>
                    <PaginationItem>
                      <PaginationNext
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          setExpensePage((p) =>
                            Math.min(Math.ceil(tripExpenses.length / ITEMS_PER_PAGE), p + 1)
                          );
                        }}
                        className={
                          expensePage === Math.ceil(tripExpenses.length / ITEMS_PER_PAGE)
                            ? 'pointer-events-none opacity-50'
                            : ''
                        }
                      />
