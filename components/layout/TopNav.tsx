        ...
        <Popover>
          ...
          <PopoverContent align="end" className="w-80 p-0 bg-popover border-border shadow-xl">
            <div className="flex items-center justify-between p-4 border-b border-border">
              ...
              <Badge variant="secondary" className="text-xs bg-primary/10 text-primary">{unreadCount} New</Badge>
            </div>
                  ...
                  {notifications.map((notif) => (
                    <div key={notif.id} className="p-4 border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                      <div className="flex justify-between items-start mb-1 gap-2">
                        <h5 className={`text-sm font-medium ${notif.type === 'warning' ? 'text-orange-500' : 'text-foreground'}`}>
                          {notif.title}
                        </h5>
                        <Button 
                          variant="outline" 
                        ...
                        >
                          {notif.id.startsWith('driver-') ? 'Renew' : notif.id.startsWith('vehicle-') ? 'Available' : 'Dismiss'}
                        </Button>
                      </div>
